...
while True:
    print("--------------------------")
    print("Serveur en écoute sur l'adresse IP", local_ip, "ou 127.0.0.1")
    ...
    for file in d.rglob('*'): 
        if file.name == fichier and file.is_file():
            ...
            #formatage de la réponse si le fichier est un fichier txt
            try:
                reponse = (
                "HTTP/1.1 200 OK\r\n"
            ...
            print("-----------------------------")
            print(reponse)
            connectionSocket.sendall(reponse.encode('utf-8'))
            envoi = True
            break
